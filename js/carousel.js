...
class Carousel {
  constructor(el) {
    this.el = el;
    this.carouselOptions = ["previous", "play", "next"]; // "add",
    this.carouselData = [
      {
        id: "1",
  ...
  setControls(controls) {
    controls.forEach((control) => {
      control.onclick = (event) => {
        ...
        // Gerencie as ações de controle, atualize primeiro os dados do nosso carrossel e, em seguida, com um retorno de chamada, atualize nosso DOM.
  ...
  previous() {
    ...
    this.carouselInView.forEach((item, index) => {
      this.carouselContainer.children[index].className =
        `carousel-item carousel-item-${item}`;
    });

    // Using the first 5 items in data array update content of carousel items in view
    // Utilizando os 5 primeiros itens da matriz de dados, atualize o conteúdo dos itens do carrossel em exibição.
